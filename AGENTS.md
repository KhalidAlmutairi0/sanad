...
5. **Human gates.** Nothing enters the citation store without `verified_by` set by a human. Scores compute only over findings with `review_status IN ('accepted','rejected')` — pending findings never affect scores.
